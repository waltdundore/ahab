...
### #0: The Transparency Principle (MANDATORY)

**Every teachable moment not utilized is a wasted opportunity.**

When you run a make command, you should see:
1. What command you typed
2. What it's actually running underneath
3. Why it's running that way

**This isn't just good UX - it's education. Users learn by seeing the real commands.**

#### Make Target Pattern (MANDATORY)

Every make target MUST show what it's running:

```makefile
.PHONY: test
test:
	@echo "→ Running: ./tests/test-workstation-bootstrap.sh"
	@echo "   Purpose: Validate workstation creation and provisioning"
	@./tests/test-workstation-bootstrap.sh
...
install-service:
	@echo "→ Running: ansible-playbook playbooks/deploy-service.yml -e service=$(SERVICE)"
	@echo "   Purpose: Deploy $(SERVICE) with security hardening"
	@ansible-playbook playbooks/deploy-service.yml -e service=$(SERVICE)
```

**Why this matters**:
- Users see the real commands (transparency)
- Users learn what's happening (education)
- Users can run commands manually if needed (empowerment)
- Users understand the system (confidence)
- Every interaction is a teaching moment (opportunity)

**Examples**:
```bash
# ❌ BAD: Silent execution (no learning)
$ make test
[test output]

# ✅ GOOD: Transparent execution (learning happens)
$ make test
→ Running: ./tests/test-workstation-bootstrap.sh
   Purpose: Validate workstation creation and provisioning
[test output]
```

**This is not optional. This is how we teach.**
