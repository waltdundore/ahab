...
setup_submodule() {
    ...
    # Use the shared function from setup-common.sh
    setup_git_submodule "$SECRETS_REPO_URL" "$SECRETS_DIR"
}

create_sanitized_examples_wrapper() {
    print_section "Creating sanitized example files"
    
    # Use the shared function from setup-common.sh
    create_sanitized_examples "${FILES_TO_MIGRATE[@]}"
}
...
migrate_files_to_secrets() {
    ...
    # Use the shared function from setup-common.sh
    migrate_files_to_secrets_repo "$SECRETS_DIR" "$BACKUP_DIR" "${FILES_TO_MIGRATE[@]}"
}
