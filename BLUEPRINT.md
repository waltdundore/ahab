...
|---|---|---|---|
| D-01 | Vault pass: `~/.config/ansible` vs `repo/secrets/.vault_pass` vs `/nas/secrets/ansible` — **4th variant minted on hub 2026-09-10** (D-21). **LIVE COPY FOUND by `bin/claim-eval.py`, same day**: hub working tree `secrets/.vault_pass`, 5 bytes, mode **604 (world-readable)**, gitignored ⇒ invisible to every `git status` ever run there. Trunk already references this path (`setup-control-node.yml:76`) | resolver in setup-control-node.yml (done, ungated); NAS mount real on all boxes (D-02); copy must drop per L3 | **FAILING — blocking row in the claim registry** |
| D-02 | `/nas` on d701 is a LOCAL DIR, not the NFS share from storage.dundore.net — "same path, two trees, one stale" (audit 2026-08-21). **Confirmed on the hub too** 2026-09-10: `stat -f -c %T /nas` → `xfs` (local), so this is the fleet-wide default, not a d701 quirk | nfs role mounts storage.dundore.net on every fleet node; assert mountpoint fstype=nfs4 | TODO — gates all vault capability (D-06) |
...
| D-06 | ~~two vault variants~~ CONFIRMED same secret (operator 2026-09-09); canonical = /nas/secrets/ansible/vault_pass. **Canonical file absent on both control nodes probed** (hub 2026-09-10, sager 2026-09-11) — but **not because no secret exists**: the hub holds a working plaintext copy at the D-01 path (5 bytes, 604). So vault operations were possible on the hub all along, and a session that minted its own password (D-21) did so out of *not probing*, not out of necessity | resolver merged (37d6a90); move the secret to /nas when L5 NFS lands (D-02), then the repo copy drops (L3) | CODE DONE, WAIT L5 — **unlock is D-02, never a competing password** |
...
| D-16 | SELinux unaccounted in Ansible: file-level tasks (`copy`/`template`/shell-pastes) on enforcing Fedora leave wrong contexts — the failure is silent (no "bad ownership" line in `/var/log/secure`, just `preauth` close); sager's lockout twin (0-byte `authorized_keys` + near-miss labeling) is this exact class | selinux-aware modules (`ansible.posix.authorized_key`, `file`+`sefcontext`) or explicit `restorecon` on every managed path; context asserts in verify tasks | TODO (operator-raised 2026-09-10). **Reproduced 2026-09-10 on hub**: unlabeled `authorized_keys` ⇒ sshd closes at `preauth` with no error line; `restorecon` fixes it, a context assert makes it loud |
| D-21 | A **new random vault password** was minted at `~/.config/ansible/vault_pass` on the hub by an agent session that had not read D-06 — law §6's canonical-path rule violated by the very session doing identity work. Root cause is procedural: **an agent can write tier-2 code and mint secrets without ever reading layer 1** (B-016) | deleted same session (verified absent). Structural teeth: L0 refuses to run when the D-06 canonical vault file is missing or a competing variant exists | CREATED + REVERTED 2026-09-10; structural fix TODO |
| D-22 | A **second fleet identity** (`fleet-mgmt@2026-09-10`, ed25519) was minted and hand-installed into the **human** `wdundore` account on 9 machines — violates the operator ruling that Ansible never connects as a human, SPEC §2 (`wdundore`/`waltdundore`/`root` direct use prohibited), and forks identity away from the canonical `keys/service_id.pub` (live on sager per B-002) | revoke by code: `ansible.posix.authorized_key: state=absent` matched on comment `fleet-mgmt@2026-09-10`, per host, + `restorecon` assert; local residues `~/.ssh/fleet/` and empty `~/.config/ansible/keys/` on hub (deletion blocked by the operator safety net, correctly) | **OPEN — not executable: 0/9 hosts reachable (B-002). Gated, not optional** |
| D-23 | `playbooks/distribute-key.yml` — the play its own header calls the way to push keys to an already-managed host — could **never run**: `include_tasks: file: roles/common/tasks/setup_ssh.yml` resolves against `playbooks/`, so it died with "Could not find or access `playbooks/roles/common/tasks/setup_ssh.yml`". `roles/bootstrap/tasks/main.yml:27` already uses the correct `role_path` form for the same role | `file: "{{ playbook_dir }}/../roles/common/tasks/setup_ssh.yml"` (`homelab@feature/l0-identity-evidence-20260910`); re-run `changed=1` then `changed=0` | FIXED IN PR, unaudited |
| D-24 | Same play's header cites `mgmt_ssh_pub_key_files` in `group_vars/all.yml` (P4-01) — **`group_vars/` does not exist in the repo**, so the advertised management key list is dangling and silently falls back to `['keys/service_id.pub']`. Correct by accident; the opencode agent key the comment promises is included nowhere | needs a decision on what the mgmt key list *is* (tier-2 values), then `group_vars/all.yml` in code — do not patch the fallback | REGISTERED, unfixed by design |
...
## Live-probed facts (2026-09-10, hub L0 identity probe)

Vantage: the probing session **runs on the hub** (10.200.10.20). Evidence:
`dundore-homelab@feature/l0-identity-evidence-20260910:tests/evidence/2026-09-10-l0-identity-probe.md`.

| Item | Status | Evidence |
|---|---|---|
| `ansible_user` exists on hub; a key installed for it authenticates | LIVE-PROBED (localhost only) | uid 1002; `ssh -i <key> ansible_user@localhost` → `KEY_LOGIN_OK`; contexts `ssh_home_t` after `restorecon` |
| fleet reachability from hub with keys this node holds | **FAIL 0/9** | LAN + tailnet + MagicDNS, users `wdundore`/`ansible_user`, all denied; tailscale SSH not enabled on nodes |
| **L4 vagrant gate has no runner on the hub** | **FAIL** | no `vagrant`/`libvirt`/`qemu`/`virsh` ⇒ law 1 (`vagrant → test → deploy`) is unexecutable from this vantage (B-015) |
| `/nas` on hub | LIVE-PROBED = local `xfs` | D-02 is fleet-wide, see register |
| D-06 canonical vault file on hub / sager | **ABSENT on both** | `ls /nas/secrets/ansible/vault_pass` → No such file |
| hub → GitHub | PARTIAL | SSH read OK (`ls-remote`: homelab `9d6e4a3`, ahab `412cefc`); **first push from hub landed** (homelab `feature/l0-identity-evidence-20260910` = `45085f4`); no `gh` CLI here ⇒ PRs opened by operator or from the laptop |
| AWX shape (refines B-014) | LIVE-PROBED | web **is reachable** on hub: `:8043` HTTPS-only (`400 plain HTTP` = correct nginx behavior), plain HTTP `:8013` → 200 UI. Project `dundore-homelab` pinned to **retired branch `production`** @ `ee408b5e3`; only `Demo Inventory`; JT `d701-update` → `main.yml` on Demo Inventory |

## Live-probed facts (2026-09-10, first `claim-eval` baseline — 19 claims)

Evidence: `homelab@feature/l0-identity-evidence-20260910:tests/evidence/claims-verdicts.json`
(+ append-only `claims-history.jsonl`). 8 PASS · 7 FAIL · 2 NOT-MINE · 1 UNPROBED · 1 POLICY · 2 FLIP.

| Item | Status | Evidence |
|---|---|---|
| D-01 live plaintext copy on hub (`secrets/.vault_pass`, 5 bytes, **604**) | **FAILING** (blocking) | `find secrets -type f -perm -o=r` non-empty; invisible to `git status` (gitignored) |
| Gitea still at its **install page**, never initialized | LIVE-PROBED (corroborates CONTEXT §2) | `https://gitea.dundore.net/` → `<title>Installation - Gitea</title>`; `/install`, `/login`, `/api/v1/version` all 404 |
| dev Kuma serves 200 over traefik (not tunnel-only) | LIVE-PROBED | `https://uptime.dundore.net/dashboard` → 200 from hub |
| `spark1.dundore.net` **does not resolve from the hub** | **FAILING** — vLLM head unreachable by name at this vantage (README §7 claim) | `getent hosts spark1.dundore.net` → no answer; `curl :8000/v1/models` → rc=6 |
| NetBox never deployed | LIVE-PROBED | `getent hosts netbox.dundore.net` → no answer (negative claim, `mode: absent`) |
| `d701 = prod` identity | **NOT-MINE** by design | declared `vantage: console`; hub SSH to .10 is key-denied (B-002). A roles law probed from the wrong vantage is how roles got inverted once already |
| operator-asserted "vault password == the other passwords" | **UNPROBED, permanently** | no machine assertion corresponds to a shared *string*; recorded as a source, not a verdict (law §8) |
...
| B-002 | **sager CLOSED 2026-09-10 — unlocked & audited** (root-pasted `authorized_keys` + restorecon; `ansible_user`+`service_id` LIVE, hostname-probed) — **hub still unmanaged, and 0/9 hosts accept a key held by the hub session (2026-09-10)** | was: dev leg + hub recovery blocked; hub leg remains, **plus every remote leg out of the hub remains** | CONSOLE on hub only: inject `keys/service_id.pub` for ansible_user; proven recipe: `tee -a ~/.ssh/authorized_keys` + 700/600 + `restorecon` (see D-16). The recipe is sound and **the hub leg is now closed by code, not console** (2026-09-10): `distribute-key.yml` + canonical `keys/service_id.pub` installed the key for `ansible_user` (`authorized_keys` 600 `ssh_home_t`, byte-identical to the tracked pub, re-run `changed=0`). What remains: *remote* first touch on the other 8, and authentication proof, which needs a node holding the private half (P4-02 — lost from this repo; sager holds a working pair per this row) |
...
| B-014 | AWX web is **live on the hub** (2026-09-10: `:8013` HTTP 200, `:8043` HTTPS) — B-001's "no web port exposed" is partly superseded — but it remains a dev-source `devel` checkout, unpinned (D-20); `awx.dundore.net` DNS+ingress vanished; shell-history OAuth tokens burned; and its objects are **hand-built drift**: Project pinned to retired `production` @ `ee408b5e3`, only `Demo Inventory`, JT `d701-update` targeting Demo Inventory. It is also **unmonitored**, so by law 2 this controller does not exist | webhook→deploy→drift legs stay untestable: the SCM ref is a branch the branch law retired, and no real inventory or machine credential exists to run with | bring AWX up **from codified bring-up, not history** (D-20): pin version, web via traefik + dnscontrol record, migrate→superuser→fresh vault-stored token, `/api/v2/ping` smoke, kuma monitor; then **awx-as-code** replaces `Demo Project`/`d701-update` and repoints the Project to `prod` |
| B-015 | **no L4 gate runner exists.** The hub has no vagrant/libvirt/qemu, and no reachable node is attested as a runner (D-15 places the libvirt Vagrantfile on d701, which the hub cannot SSH) | law 1 (`vagrant → test → deploy`) and SPEC §2 ("each extraction is proven in Vagrant first") cannot be satisfied ⇒ **no L0 work is legal from this vantage**, physical or otherwise | declare and converge a runner (libvirt/QEMU + `vagrant-libvirt`, per the open-source-only law) **as code** on a host the operator names; ad-hoc install is banned by law §7 |
| B-016 | **agents act without reading layer 1.** This session wrote tier-2 L0 code, minted a competing vault password (D-21) and a second fleet identity (D-22) while `BLUEPRINT.md` + `SPEC.md` sat unread in a local clone. The flawed feedback loop the operator named is exactly this: nothing forces a layer-1 read, and no second party gates agent output | every future session can repeat it; PM self-grading dressed localhost-only probes as "verified" | structural: (a) L0 refuses to run unless the D-06 canonical vault is present (D-21 teeth); (b) every builder brief cites the BLUEPRINT rows it serves; (c) spark-auditor gates L0 before any physical box (B-010); **(d) LANDED 2026-09-10 — homelab law §8 "Assertion law" + `claims/claims.yml` + `bin/claim-eval.py`: an assertion carries a probe, a vantage and a ttl, and `--require <id>` is the preflight any agent or CI runs before quoting one.** Its first run contradicted this file (D-01 live copy) and its own author (see D-06) within minutes, which is the evidence that the gate is real rather than aspirational |
...
7. `homelab@45085f4` + the 2026-09-10 hub probe rows above — spark-auditor reproduces the two load-bearing claims independently (0/9 host reachability; hub `/nas` fstype = `xfs`) and proves D-22's keys are gone once B-002 unlocks, instead of taking this agent's word for either
