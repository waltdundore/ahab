...
| B-017 | **Vagrant gate cannot run on sager (D-25).** Lab-net NAT/forward rules absent and unowned by code; also a second, still-unfixed Tier-A defect found 2026-09-11: `ci.yml` triggers on `dev`, but this repo's trunk pair on the forge is `prod`+`development` — and job 2 keys off `refs/heads/development`, so the health job can never fire | M0 round 2 + every future L4 gate is blocked; the Tier-A gate is inert for TWO independent reasons | operator ruling on posture (enable firewalld vs converge narrow lab-net rules — PM recommends narrow rules, zero change to existing service exposure); then role + converge + `make lab-up` green; fix ci.yml refs to the repo's real trunk |
