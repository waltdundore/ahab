...
|---|---|---|---|
| D-01 | Vault pass: `~/.config/ansible` vs `repo/secrets/.vault_pass` vs `/nas/secrets/ansible` — **a 4th variant was minted on the hub 2026-09-10** (D-21) | resolver in setup-control-node.yml (done, ungated); NAS mount real on all boxes (D-02) | CODE PARTIAL |
...
| D-06 | ~~two vault variants~~ CONFIRMED same secret (operator 2026-09-09); canonical = /nas/secrets/ansible/vault_pass. **The canonical file is absent on both control nodes probed** (hub 2026-09-10, sager 2026-09-11) ⇒ no vault capability anywhere; the secret lives only on the admin MacBook / sager until NFS lands | resolver merged (37d6a90); move file to /nas when L5 NFS lands (D-02) | CODE DONE, WAIT L5 — **unlock is D-02, never a competing password** |
